...
public class Data {
    ...
    public static int[] F1(int[] A, int[] B, int[] C, int[] D, int[][] MA, int[][] MD) {
        return addTwoVectors( // (A * B) + (C * (D * (MA * MD)))
                multiplyTwoVectors(A, B), // A * B
                multiplyTwoVectors( // C * (D * (MA * MD))
                        C,
    ...
    public static int[][] F2(int[][] MG, int[][] MH, int[][] MK, int[][] ML) {
        return addTwoMatrices( // MG + MH * (MK * ML)
                MG,
                multiplyTwoMatrices( // MH * (MK * ML)
                        MH,
                        multiplyTwoMatrices(MK, ML) // MK * ML
                )
        );
    }
    ...
    public static int[] F3(int[] P, int[][] MR, int[][] MT) {
        Arrays.sort(P); // Сортування вектора P
        return multiplyVectorAndMatrix(P, multiplyTwoMatrices(MR, MT)); // SORT(P) * (MR * MT)
    }
